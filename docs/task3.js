...
let generate=true;
function page_button(){
if (dataContainer && generate){
  //if (pictureContainers.length < image_url.length){}
  for (let i=0;i<pictureContainers.length;i++){
    let divElement = document.createElement('div');
    divElement.className='picture';
    ...
    for (let j =0;j<pictureElements.length;j++){
      ...
      spanElement1.appendChild(textNode);
      if (pictureElements[j].innerText){//如果已經有景點標題，就不需要再加一次標題
        continue;
      }
      else{
        pictureElements[j].appendChild(spanElement1);//如果沒有加過標題，就加上
      }
      //textElements[i].classList.add('text');
    }
    if (newpictureContainers.length>image_url.length){
      generate=false;//如果產出的div>索引的景點數量，就停止產出
    }
}
}  
} 

function page_button_down(elem){
  if (dataContainer){
  let button_down=document.getElementById("more-button");
  button_down.style.color="fefefe7e";
  button_down.style.backgroundColor='rgba(94, 94, 94, 0.174)';}
}

function page_button_up(elem){
  let button_down=document.getElementById("more-button");
  button_down.style.color="0000007e";
  button_down.style.backgroundColor='rgba(167, 167, 167, 0.174)';
  button_down.style.boxShadow= "0px 4px 6px rgba(0, 0, 0, 0.178)";
}