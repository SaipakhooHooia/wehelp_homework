...
for i in range(len(data['data']["results"])):
    longitude.append(data['data']["results"][i]["longitude"])#['121.508447', '121.508274', '121.530849'...
    latitude.append(data['data']["results"][i]["latitude"])
...
for i in range(len(data['data']['results'])):
    url=data['data']['results'][i]['filelist']
    pattern = r'https:\/\/.*?\.jpg'
...
for i in range(len(spot)):#共58個景點
    if list(station_num.values())[i] in spot_dict.keys():#如果station_num.values()站牌名稱已經在spot_dict.keys()裡了
        spot_dict[list(station_num.values())[i]].append(data['data']['results'][i]['stitle'])#spot_dict在該站牌名稱的key值裡新增景點名稱
    else:
        spot_dict[list(station_num.values())[i]]=[]#如果station_num.values()站牌名稱沒有在spot_dict.keys()裡，就新增這個站牌的key
        spot_dict[list(station_num.values())[i]].append(data['data']['results'][i]['stitle'])#並且在該站牌名稱的key值裡新增景點名稱
